    ...
    <>
      ...
      <div className="bg-[rgba(0,0,0,0.3)]  w-full h-full absolute top-0 left-0">
        <div className="absolute mx-auto top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 p-20 bg-[rgba(0,0,0,0.5)] w-full animate-[fadeIn_1s_ease-in-out]">
          <div className="w-[400px] mx-auto animate-[upIn_1s_ease-in-out_forwards]">
          ...
          <div className="flex justify-center items-center gap-20 mt-20">
            {nav.map(category => (
              <Link
                href={`${category.path}`}
                key={category.path}
                className="p-4 text-white border-2 bg-[rgba(0,0,0,0.2)] border-sky-500 text-2xl font-semibold rounded-lg shadow-[4px_2px_4px_0_rgba(255,255,255,0.3)] hover:border-logo_color">
                {category.category}
              </Link>
            ))}
          </div>
